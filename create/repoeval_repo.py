...
from create.create_utils import save_tsv_dict, save_file_jsonl
...
def iterate_repository(base_dir: str, repo: str) -> dict:
    ...
    for fname in files:
        try:
            ...
            fpath_tuple = tuple(
                os.path.normpath(fname).split(os.sep)[len(base_dir_list) :]
            )
    ...
    if len(skipped_files) > 0:
        print(
            f"Skipped {len(skipped_files)} out of {len(files)} files due to I/O errors"
        )
...
class RepoWindowMaker:
    def __init__(self, base_dir, repo, tasks, window_size, slice_size):
        ...
        self.slice_step = (
            1 if window_size // slice_size == 0 else window_size // slice_size
        )
        self.tasks = tasks
    ...
    def _buid_windows_for_a_file(self, fpath_tuple, code):
        ...
        for line_no in range(
            0, len(code_lines), self.slice_step
...
def main():
    ...
    if args.num_shards > 1:
        ...
        shard_size = len(REPOs) // args.num_shards
        start_idx = args.shard_id * shard_size
        end_idx = start_idx + shard_size
        if args.shard_id == args.num_shards - 1:
            end_idx = len(REPOs)
        REPOs = REPOs[start_idx:end_idx]
        print(f"Sharding repos: shard {args.shard_id}/{args.num_shards} -> repos {start_idx}:{end_idx}")

    file_name = (
        f"{args.split}_level_completion_{args.context_length}_context_codex.test.jsonl"
    )
    ...
    for repo in REPOs:
        if repo not in data_dict:
            print(f"Skipping repo with no tasks: {repo}")
            continue
        ...
        for example in data_dict[repo]:
            ...
            new_example = {
                ...
                "docs": gt[:10],
                "metadata": {
                    k: v for k, v in example["metadata"].items() if k != "ground_truth"
                },
            }
        ...
        results_file = os.path.join(
            args.results_dir,
            f"repoeval-{args.split}-{repo}-{args.context_length}-gt.jsonl",
        )
        ...
        results_file = os.path.join(
            args.results_dir,
            f"repoeval-{args.split}-{repo}-{args.context_length}-infile.jsonl",
        )
        with open(results_file, "w") as fw:
            for ex in gt_data:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--context_length", type=str, default="2k", choices=["1k", "2k", "4k"]
    )
